...
export class QrScanPage {
  ...
  postJsonData(date, time) {
    let jsonData = { card_id: this.card_id, term_id:this.term_id, date: date, time: time, qrId: this.qrId }; //สร้าง obj
    console.log(jsonData);
    // let url = 'http://192.168.1.72/servicephp/saverabsong.php?card_id=' + this.card_id + '&date=' + date + '&time=' + time + '&qrId=' + this.qrId+'&term_id=' +  this.term_id;
    // let url = 'http://192.168.1.4/servicephp/saverabsong.php?card_id=' + this.card_id + '&date=' + date + '&time=' + time + '&qrId=' + this.qrId+'&term_id=' +this.term_id; 
    let url = 'http://192.168.1.4/servicephp/saverabsong.php'; 
